...
client.on('messageCreate', async Message => {
    ...
    if (Command == 'setup') {
        ...
        const Embed = new EmbedBuilder()
            .setAuthor({ name: 'My Room Dashboard', iconURL: client.user.displayAvatarURL() })
            .setDescription(`*clicky 3la buttons tema bash t'controler room dyalk.`)
            .setTimestamp()
...
client.on('interactionCreate', async Interaction => {
    if (Interaction.isButton()) {
        ...
        if (!Channel) return Interaction.reply({ content: `*rak madkhelsh voiceRooms*.`, ephemeral: true })
        const Data = db.get(`Temporary_${Channel.id}_${Interaction.user.id}`)
        if (Data !== Channel.id) return Interaction.reply({ content: `machi nta howa mol had channel`, ephemeral: true })
